...
def buildNotionData(tmdbData, programStatus):
    ...
    properties = {
        ...
        'Éditeur': {
            'multi_select': [{'name': tmdbData["source"]}]
        },
        'Statut': {
        ...
        'Tags': {
            'multi_select': [
                {'name': genre } for genre in tmdbData["tags"].split(', ')
            ]
        },
        'Lien': {
            'url': tmdbData["homepage_link"]
        },
        'id': {
            'rich_text': [
                {
...
def userChoiceHandler(programStatus, DATABASE_ID,  HALL_TV_TOKEN, programId, media_type):
    ...
    if(resultsNumber < 1 ):
        ...
        return addProgram(DATABASE_ID,  HALL_TV_TOKEN, programId, media_type, programStatus)
