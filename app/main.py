            ...
            for entry in data["entry"]:
                for messaging_event in entry["messaging"]:
                    if messaging_event.get("message"): 
                        ...
                        logging.error("Webhook received unknown messaging_event: " + messaging_event)
                        return sendTextMessage(messaging_event["sender"]["id"], "Sorry instruction not understood ")
...
def buildTvSearchBody(senderId, results):
    ...
    elements = [
        ...
        ) for result in results if result["poster_path"] != None 
    ]
...
def callSendApi(messageData):
    ...
    try :
        ...
        logging.info(f"Message successfully sent to {recipientId}. status code : {response.status_code}")
        return response

    except Exception as e:
        logging.error(f"Failled to send message to {recipientId}. status code : {response.status_code}")
        logging.error(traceback.format_exc())
        return None


def sendTextMessage(recipientId, messageText):
    logging.info("sending message to {recipient}: {text}".format(recipient=recipientId, text=messageText.encode('utf-8')))
    messageData ={
        "recipient": {
            "id": recipientId
        },
...
def receivedPostback(event):
    ...
    logging.info("received postback from {recipient} with payload {payload}".format(recipient=recipient_id, payload=payload))
    ...
    if(userChoiceHandler(programStatus, DATABASE_ID,  HALL_TV_TOKEN, programId, "tv")):
        return sendTextMessage(sender_id, f" 5 sur 5 ! Tu peux retrouver ton programe {programStatus} dans Notion !")
    
    return sendTextMessage(sender_id, "Désolé, une erreur s'est produite, l'action n'a pas été prise en compte")
